...
function HomePage(): JSX.Element {
  ...
  const load = useCallback(async () => { const snapshot = sessionStore.getSnapshot(); if (requiresLogin(snapshot)) { void Taro.reLaunch({ url: "/pages/login/index" }); return; } setLoading(true); setError(""); try { setData(await apiClient.getAnalyticsSnapshot()); } catch (reason) { setError(errorMessage(reason)); } finally { setLoading(false); } }, []);
  usePageLoad(async () => { await load(); }); usePullDownRefresh(() => { void load().finally(() => void Taro.stopPullDownRefresh()); });
  ...
  return <Page tab><PageHeader eyebrow="学习" title={user?.displayName ? `${user.displayName}，继续加油` : "今天也来练一练"} subtitle="从最近一次练习继续，或者查看自己的薄弱点。" />
    {loading && !data ? <StateView phase="loading" title="正在整理学习数据" detail="统计与最近练习马上就好。" /> : null}{error && !data ? <StateView phase="error" title="学习数据加载失败" detail={error} actionLabel="重新加载" onAction={() => void load()} /> : null}

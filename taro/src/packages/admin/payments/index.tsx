...
function AdminPaymentsPage(): JSX.Element {
  const api = usePageApi();
  const allowed = sessionStore.getSnapshot()?.user.role === "admin";
  const [items, setItems] = useState<PaymentOrder[] | null>(null);
  const [message, setMessage] = useState("");
  const load = useCallback(async () => {
    try {
      setItems(await api.payments.adminList());
    } catch (error) {
    ...
    <PermissionGate allowed={allowed}>
      <Page>
        <PageHeader
          eyebrow="管理员"
          title="订单与退款"
          subtitle="生产支付未配置时仅展示已有订单，不会发起必然失败的购买。"
        />
            ...
            {items.length ? (
              items.map((order) => (
                <EntityCard
                  key={order.id}
                  title={`订单 #${order.id}`}
                  meta={`${order.kind} · ¥${(order.amountCents / 100).toFixed(2)}`}
                  badge={
                    <StatusTag
                      tone={
                        order.status === "paid"
              ...
              <StateView
                phase="empty"
                title="暂无订单"
                detail="测试网关验收产生的订单会显示在这里。"
              />
            )}
          </View>
        ) : (
          <StateView phase="loading" title="正在读取订单" detail="马上就好。" />
        )}
      </Page>
    </PermissionGate>
  );
...
export default protectedPage(AdminPaymentsPage);

...
function SearchPage(): JSX.Element {
  ...
  const search = async () => {
    ...
    try {
      setItems(
        (await api.search.questions({ q: query.trim(), limit: 100 })).items,
      );
    } catch (error) {
          ...
          {items.length ? (
            items.map((item) => (
              <EntityCard
                key={item.id}
                title={item.stem}
                meta={`${item.question_type_id} · ${item.status}`}
                onClick={() =>
...
export default protectedPage(SearchPage);

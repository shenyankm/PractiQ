...
- 新服务使用本地文件根目录（历史默认名 `.local/ai-oss` 保持不变，生产必须为持久挂载的绝对路径）。旧任务、旧 checkpoint 和文件保留在原环境；不自动迁移、不删除、不复用旧状态库。
- 本地文件上传和素材读取继续进行鉴权、大小与 SHA-256 校验。
...
服务仅使用本地文件，已移除 OSS SDK、凭证配置与云端清理逻辑。旧 `AI_STORAGE_BACKEND=oss` 会明确报错；未设置该变量或值为 `local` 时使用本地目录。历史默认目录名 `.local/ai-oss` 保持不变，避免现有本地数据失联；桌面数据库的旧 `oss_url` 列仅保留作备份兼容，不读取或写入连接配置。

原 OSS 部署须先在原版本完成未结束任务，再停止服务并备份。按原 objectKey 将源文件和衍生文件人工复制至持久本地目录，核对大小与 SHA-256 后设置绝对 `AI_STORAGE_DIR`，移除 `AI_STORAGE_BACKEND` 和 `AI_OSS_*` 配置，再用新版本创建任务。执行指纹变化后不恢复旧任务；保留原部署、Bucket、数据库和配置用于回退。升级代码不会访问或删除云端文件。
...
shell 须预先加载配置。盘点所有保留任务、完整 checkpoint 历史和 Store；任一读取失败立即停止。默认保留至少 187 天。`--quarantine` 必须在维护模式、排空、停止服务后执行；独占锁阻止服务同时启动，先保存可恢复清单再移动。文件隔离至同根 `.quarantine/<runId>/`，不删除原始文件内容。
...
- 文档提取在独立进程组执行，超时或取消后终止并等待进程回收，转换器子进程也在此组内。临时目录由父进程清理。存储使用独立有界线程池；等待超时不释放仍在工作的 I/O 槽位，满载时有界等待，达到存储等待期限返回 `OBJECT_STORE_UNAVAILABLE`，底层工作结束后恢复容量。

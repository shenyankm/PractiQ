...
`LLM_MODEL` 必填，统一指定支持文本及图片输入的模型。旧的 `LLM_TEXT_MODEL` / `LLM_VISION_MODEL` 不再读取，请将原视觉模型 ID（缺失时选原文本模型）填入 `LLM_MODEL`。TXT/CSV 通过文本输入提题；PDF/图片通过图片输入直接提题。PDF 由 PDFium 渲染；部署镜像包含中文字体，不依赖办公软件。Word（.doc/.docx）请先在 Word 或 WPS 中导出为 PDF。
...
评分需要完整的简答题、非空作答，以及参考答案或评分细则。解析结果中的 `sourceScore`、`scoringRubric`、`scoreSourceText` 分别记录原卷分值、评分细则和分值原文，缺失时为 `null`。新版 JSON 可省略这些可选字段；解析不会生成答案或评分依据。
...
评分统一使用 `LLM_MODEL`，有图片时向同一模型附带图片输入。评分复用模型并发、限流、有限纠错和用量记录；文档任务的总预算和运行期限不适用于该接口。提供原卷细则和分值时，服务按原满分评分，再换算成本次满分。个人自测以外的使用需要独立标注与校准，当前验证边界见[考试验收记录](../../app/EXAM_ACCEPTANCE.md)。
...
本地存储、任务调度和提取进程传输的合成压测见[性能验证记录](../reports/performance-20260921.md)。服务启动时为 schema 1 的任务库补建排序索引；业务读取复用独立连接并串行结束游标，写事务、checkpoint 和 Store 仍各自隔离。调度器由提交、控制、完成和停止事件唤醒，启动时扫描恢复队列。
...
- PDF（含文字版）的全部页面均以约 200 DPI 转图，配置的模型必须支持图片输入。页面图像直接输出结构化题目、分组和图形，不生成 OCR 文本、不再经过文本分片提题。每页携带相邻页作为上下文，只输出起始于本页的题目；超出窗口的续文保留缺失字段，不猜测。失败页单独补跑并重新合并，成功页不重算。页内配图由模型提供边界框，Pillow 执行裁剪。页内裁剪图以描述前缀 `[page crop]` 标注来源。页图可并行识别；单个视觉或文本分片失败会保留明细并返回 `PARTIAL`。
- Graph State 只保存对象引用和结构化结果，不保存文件 Base64。
- 模型输出经过 Pydantic 校验；失败调用受统一重试与并发上限约束。

解析结果中的素材可同时包含 `imageRef`（裁剪图）和可选的 `sourceRef`（完整来源页），两者均为 `ArtifactReference`，通过同一素材读取接口鉴权读取并校验大小及摘要。新版 JSON 可省略 `sourceRef`；客户端应保留该引用，供用户核对裁剪边缘与上下文。完整来源页可能含参考答案，桌面在自测和模考交卷前移除原页引用并隐藏查看入口。

简单表格由模型提供矩形单元格数组，服务转义竖线并生成 Markdown，保留单元格内容及原文转录。无法忠实表示的合并单元格、多级表头或不规则行保留图片和可读文字，并标记复核，不补造单元格。跨页内容仍受相邻页窗口限制，不保证完整重建；部分失败结果保留失败页的原页引用供复核。

## PDF 全页视觉解析运行要求

PDFium 在可终止的隔离进程中渲染页面；页图和裁剪图进入素材存储，临时文件在提取结束后清理。页数、像素、视觉总字节和裁剪上限仍适用；全页识别会增加模型调用成本与耗时。

提取进程以二进制页图和有界清单交付结果，主进程在线程中读写临时文件；取消时先等待正在进行的文件操作结束，再清理目录。此内部协议须随内置 Python 服务一起打包，外部任务与素材 API 不变。

...
五个 Graph 共用 [任务 API 与恢复规则](document-tasks.md)。默认仍返回 `PARTIAL`；`failurePolicy="review"` 在处理失败时等待补跑或接受部分结果。文档输入与最终结果结构保留。所有变更请求使用 UUID `requestId`；恢复使用最新 `checkpointId`，暂停/中断使用目标 `runId`。请求受理与实际停止分开查询。
...
版本化报告记录本地真实服务结果，不代表生产 SLO。当前评测集有 25 份公开合成文档；以下历史报告对应各自的数据集版本；使用规则评分、人工金标和不回退门禁。数据集、指标、基线比较与坏例修复流程见 [`docs/evaluation.md`](evaluation.md)。
...
图片在送模型与解码裁剪前执行 `AI_MAX_VISION_PAGE_PIXELS` 校验；直接上传多帧图片明确拒绝（`IMAGE_MULTIFRAME_UNSUPPORTED`），请拆成单帧文档。XLSX 忽略声明的工作表维度，按实际单元格读取；每表最多 10000 行、1000 列、1000000 个迭代单元格，超限表报告 `XLSX_SHEET_TOO_LARGE`，不静默漏题。

不完整答案允许缺值，但仍校验容器、元素类型、长度与引用；图片描述和标签在模型纠错边界就执行最终结果约束。默认工具调用协议的纠错历史包含匹配的工具应答，失败调用用量仍保留。XLSX 离线轨迹按 `GroundedSheetResult` 与准备阶段校验。文本跨片分组仅在共享已确认的重叠题、标题和材料说明完全一致且匹配唯一时合并；同名不同来源分组保留独立。

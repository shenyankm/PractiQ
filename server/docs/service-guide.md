...
解析结果中的素材可同时包含 `imageRef`（裁剪图）和可选的 `sourceRef`（完整来源页），两者均为 `ArtifactReference`，通过同一素材读取接口鉴权读取并校验大小及摘要。旧 JSON 可省略 `sourceRef`；客户端应保留该引用，供用户核对裁剪边缘与上下文。完整来源页可能含参考答案，桌面在自测和模考交卷前移除原页引用并隐藏查看入口。

...
本文说明独立 AI 服务的配置、文档解析、主观题评分接口与工程验证。桌面使用步骤见[项目说明](../../README.zh-CN.md)。命令默认从仓库根目录执行；历史报告仅代表对应版本的结果。
...
评分统一使用 `LLM_MODEL`，有图片时向同一模型附带图片输入。评分复用模型并发、限流、有限纠错和用量记录；文档任务的总预算和运行期限不适用于该接口。提供原卷细则和分值时，服务按原满分评分，再换算成本次满分。个人自测以外的使用需要独立标注与校准。

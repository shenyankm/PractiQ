...
COMMENT ON COLUMN users.plus_expires_at IS 'Plus 付费会员到期时间，空值表示非按期付费的 Plus';

CREATE TABLE alipay_payment_orders (
    id BIGINT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
    ...
    created_at TIMESTAMPTZ NOT NULL DEFAULT NOW(),
    updated_at TIMESTAMPTZ NOT NULL DEFAULT NOW(),
    CONSTRAINT uq_alipay_payment_orders_out_trade_no UNIQUE (out_trade_no),
    CONSTRAINT chk_alipay_payment_orders_amount CHECK (amount_cents > 0),
    CONSTRAINT chk_alipay_payment_orders_currency CHECK (currency = 'CNY'),
    CONSTRAINT chk_alipay_payment_orders_status CHECK (status IN ('created', 'paying', 'paid', 'closed', 'failed')),
    CONSTRAINT chk_alipay_payment_orders_membership_period CHECK (
...
CREATE INDEX idx_alipay_payment_orders_user_created ON alipay_payment_orders (user_id, created_at DESC);
CREATE INDEX idx_alipay_payment_orders_trade_no ON alipay_payment_orders (trade_no) WHERE trade_no IS NOT NULL;
CREATE INDEX idx_alipay_payment_orders_status ON alipay_payment_orders (status, created_at DESC);
...
COMMENT ON TABLE alipay_payment_orders IS '支付宝电脑网站支付订单';
COMMENT ON COLUMN alipay_payment_orders.out_trade_no IS '商户订单号，传给支付宝且全局唯一';
COMMENT ON COLUMN alipay_payment_orders.trade_no IS '支付宝交易号';
COMMENT ON COLUMN alipay_payment_orders.status IS '本地支付状态';
COMMENT ON COLUMN alipay_payment_orders.alipay_trade_status IS '支付宝交易状态';

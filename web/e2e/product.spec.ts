...
test('create, edit, practice and render at mobile/tablet/desktop sizes', async ({ page, request }) => {
  ...
  await page.getByRole('option', { name: '选择题', exact: true }).click();
  await page.getByRole('button', { name: '选择方式' }).click();
  await page.getByRole('option', { name: '单选', exact: true }).click();
  await page.getByRole('button', { name: '添加选项', exact: true }).click();
  await page.getByRole('button', { name: '添加选项', exact: true }).click();
  await page.getByLabel('题干', { exact: true }).fill('1 + 1 等于多少？');
  await page.getByLabel('来源原文', { exact: true }).fill('1 + 1 等于多少？A. 2 B. 3');
  await page.getByLabel('解析', { exact: true }).fill('1 加 1 得到 2。');
  await page.getByLabel('第 1 个选项内容', { exact: true }).fill('2');
  await page.getByLabel('第 2 个选项内容', { exact: true }).fill('3');

...
export function Empty({ children = '暂无内容，创建第一条记录开始吧。' }: { children?: ReactNode }) { return <Typography.Paragraph className="max-w-full overflow-x-auto">{children}</Typography.Paragraph>; }
...
export function SaveForm({ action, onSave, children, label = '保存' }: { action: ReturnType<typeof useAction>; onSave: () => Promise<unknown>; children: ReactNode; label?: string }) { return <Form className="flex flex-col gap-4" onSubmit={e => { e.preventDefault(); void action.run(onSave); }}>{children}{action.error && <Notice>{action.error}</Notice>}<Button type="submit" isDisabled={action.busy}>{action.busy ? '处理中…' : label}</Button></Form>; }

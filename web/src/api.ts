...
export type Mode = 'choice' | 'true_false' | 'fill_blank' | 'short_answer';
export interface Bank { id: number; subject_id: string; name: string; description: string; is_favorite: boolean }
export interface Option { id: number; option_label: string; content: string; sort_order: number }
...
export interface Question { id: number; question_id?: number; bank_id: number; stem: string; analysis?: string; answer_mode: Mode; choice_variant: 'single' | 'multiple' | null; question_type_id: string; status: string; question_status?: string; group_id: number | null; subset_id: number | null; options: Option[]; answer_keys?: Key[]; content_blocks?: Block[]; media?: Media[]; knowledge_points?: Knowledge[]; group?: Group; result?: AnswerResult | null }
...
export interface Task { id: number; source_question_id?: number | null; kind: string; status: string; attempt: number; result: { summary?: string; canonicalAnswer?: string; explanation?: string; answerPayload?: Record<string, unknown>; recommendations?: string[]; steps?: string[] } | null; error: { code: string; message: string } | null; usage?: { attempt: number; usage: Record<string, unknown> }[] }
...
export function answerPayload(mode: Mode, value: string, selected: string[]): Record<string, unknown> {

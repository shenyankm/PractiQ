...
export interface Item { id: number; side: 'left' | 'right' | null; content: string; sort_order: number }
...
export interface Question { id: number; question_id?: number; bank_id: number; stem: string; analysis?: string; answer_mode: Mode; choice_variant: 'single' | 'multiple' | null; matching_variant?: 'one_to_one' | 'many_to_one' | null; blank_count?: number; question_type_id: string; status: string; question_status?: string; group_id: number | null; subset_id: number | null; options: Option[]; items?: Item[]; answer_keys?: Key[]; content_blocks?: Block[]; media?: Media[]; knowledge_points?: Knowledge[]; group?: Group; result?: AnswerResult | null }

...
CREATE TABLE short_answer_questions(question_id TEXT PRIMARY KEY REFERENCES questions(id) ON DELETE CASCADE,answer TEXT NOT NULL CHECK(json_valid(answer) AND json_type(answer) IN('text','null')));
...
CREATE TABLE question_items(question_id TEXT NOT NULL REFERENCES questions(id) ON DELETE CASCADE,position INTEGER NOT NULL,item_id INTEGER,side TEXT CHECK(side IN('left','right')),content TEXT,PRIMARY KEY(question_id,position));

...
a = Analysis([str(root/'app/scripts/sidecar-entry.py')], pathex=[str(root/'server/src')], binaries=binaries, datas=datas, hiddenimports=hiddenimports, excludes=['langgraph.store.postgres','langgraph.checkpoint.postgres','psycopg','psycopg_pool','openpyxl','docx','pytest','IPython','tkinter','matplotlib'])
pyz = PYZ(a.pure)

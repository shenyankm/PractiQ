...
it("keeps JSON import usable without models and preserves the destination bank", async () => {
  ...
  vi.mocked(api).mockImplementation(async (request) => {
    switch (request.type) {
      case "banks": return [{ id: "bank-1", title: "现有题库", description: "", count: 0 }] as never;
      case "sessions": case "questions": return [] as never;
      case "info": return { version: "test", dataDirectory: "/tmp/test" } as never;
      case "pick_import": return { ticket: "ticket", title: "新文件", count: 1, reviewCount: 0, assetCount: 0, missingAssets: [], warnings: [], status: "SUCCEEDED" } as never;
      case "import": return { bankId: "bank-1", count: 1, duplicate: false } as never;
  ...
  await waitFor(() => expect(screen.getAllByText("请先配置模型地址")).toHaveLength(1));
  expect(screen.queryByRole("alert")).toBeNull();
  await userEvent.click(screen.getByRole("button", { name: "选择题库 JSON" }));
  const dialog = await screen.findByRole("dialog");
  expect(within(dialog).getByRole("combobox").textContent).toContain("现有题库");
  await waitFor(() => expect(within(dialog).getByRole("button", { name: "确认导入" }).hasAttribute("disabled")).toBe(false));
  await userEvent.click(within(dialog).getByRole("button", { name: "确认导入" }));
  await waitFor(() => expect(api).toHaveBeenCalledWith({ type: "import", ticket: "ticket", bank_id: "bank-1", title: "新文件" }));
  expect(vi.mocked(invoke).mock.calls.every(([command, args]) => command === "ai_request" && (args as { request: { type: string } }).request.type === "list")).toBe(true);
});

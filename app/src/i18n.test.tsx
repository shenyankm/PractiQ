...
it("switches through the sidebar and retains language on save failure", async () => {
  ...
  expect(screen.getByRole("dialog", { name: "语言" })).toBeTruthy();
  let select = screen.getByRole("combobox", { name: "语言" });
  ...
  select = screen.getByRole("combobox", { name: "语言" });

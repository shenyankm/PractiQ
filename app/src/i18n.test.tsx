...
it("selects language in an anchored menu and retains it on failure", async () => {
  ...
  expect(screen.getByRole("menuitemradio", {name:"简体中文"}).getAttribute("aria-checked")).toBe("true");
  ...
  failSave=true; await userEvent.click(trigger);
  ...
  expect(vi.mocked(invoke).mock.calls.some(([name])=>name==="ai_request")).toBe(false);
});

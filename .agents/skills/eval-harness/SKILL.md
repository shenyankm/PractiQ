...
## Local PractiQ evaluation

Use the existing `server/scripts/evaluate.py` workflow, not an external harness:

```sh
cd server
/path/to/python3.14 scripts/evaluate.py --validate-only
```

`make verify AI_PYTHON=/path/to/python3.14` validates fixtures and collects deterministic probe reports. See `server/docs/evaluation.md` from the repository root for quality runs. Real model runs need explicit authorization and a configured provider. No JavaScript capsule/containment utilities ship in this repository.

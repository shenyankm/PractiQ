...
conda activate langgragh  # Python 3.14; do not create a new .venv
make server-install
npm --prefix taro ci
docker compose up -d postgres redis # product infrastructure
...
For standalone production, supply Agent Server licensing secrets and use `docker compose --profile ai up -d --build`; it provides a separate Agent Server PostgreSQL volume and Redis DB 1. Its host port is also 8090, so do not run it alongside `make server-dev`. Pin the validated image digest for deployment. Local `langgraph dev` requires no production license. Do not expose the AI port outside the trusted backend network in production.

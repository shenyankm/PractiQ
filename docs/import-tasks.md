...
Local batch history has its own 20-batch pages; active batches remain visible on every history page. Import failures for the visible tasks and the selected task are resolved independently of the history page. Receipt reconciliation reuses one SQLite connection per page. Manifest discovery still scans the local batch directory; it does not load every historical item into the UI.

...
test('language defaults to English and only accepts supported values', () => {
  ...
  assert.equal(systemLanguage('zh-Hant-TW'), 'zh-TW');
...
test('Traditional Chinese is derived from Simplified and Japanese falls back to English', () => {
  assert.equal(translate('zh-TW', 'Settings', '设置'), '設置');
  assert.equal(questionTypeLabel('single_choice', 'zh-TW'), '單選題');
  assert.equal(importStatusText('stage:writing:12', 'zh-TW'), '正在寫入 12 道題');
  assert.equal(questionTypeLabel('single_choice', 'ja'), '単一選択');
  assert.equal(importStatusLabel('retry_wait', 'ja'), '再試行待ち');
  assert.equal(translate('ja', 'Untranslated string', '未翻译'), 'Untranslated string');
});
...
test('Chinese dates do not depend on platform locale data', () => {
  ...
  assert.equal(formatDay('2026-07-18', 'zh-TW'), '7月18日');
  assert.equal(formatDay('2026-07-18', 'ja'), '7月18日');
});

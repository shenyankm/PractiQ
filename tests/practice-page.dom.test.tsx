...
describe('PracticeSetupForm', () => {
  it('renders all expected practice modes and type controls', () => {
    ...
    expect(modeSelect.textContent).toContain('全量练习');
    expect(container.querySelector('select[name="mode"]')?.textContent).toContain('错题集练习');
    expect(container.querySelector('select[name="mode"]')?.textContent).toContain('按题型练习');
    expect(container.querySelector('select[name="mode"]')?.textContent).toContain('自测模考');
    expect(container.querySelector('select[name="questionTypeId"]')?.textContent).toContain('math_calculation (8)');

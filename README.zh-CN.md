...
PractiQ 包含离线桌面练习应用和可独立部署的 AI 服务。你可以直接导入已有 PractiQ JSON，也可以配置支持文本及图片输入的模型，从文档中提取题目。导入后，跨题库组卷、复习作答，并按需使用 AI 为符合条件的简答题评分。
...
桌面应用支持简体中文和英文，可通过侧边栏设置按钮上方的**语言**选择器即时切换。首次启动时中文系统使用简体中文，其余使用英文；手动选择保存在本机并随备份恢复。切换不调用模型，也不翻译导入的题目和答案。
...
命令会先构建内置 Python 服务，再启动桌面应用。导入 JSON 和离线练习无需模型凭据。需要解析或 AI 评分时，在**设置**中填写服务地址、模型 ID 和 API Key。解析与评分共用该模型，模型须支持文本及图片输入。
...
桌面语言浏览器回归：`cd app && npx playwright install chromium --only-shell && npm run test:browser`，验证 960px 英文布局及真实键盘语言切换，模拟 Tauri 边界，不调用模型；脚本自行使用端口 1420 启停 Vite。语言竞态、持久化、备份恢复及双语流程测试纳入 `make app-check`。
